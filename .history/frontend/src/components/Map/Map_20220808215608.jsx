...
export default function Map(props) {
    ...
    const lat = brewery.longitude
    const lng = brewery.latitude
 ...
 <SearchPage panTo = {panTo}/>

    return <div className="mapContainer">
        <h3 className="maph3">Breweries <span role='img' aria-label="brewery">🍺</span></h3>
        <GoogleMap 
      ...
      >
        ...
        onCloseClick={()=> {
          ...
          <div>
            <h2>{selected.name}</h2>
            <p><Link to={`/brewery/${brewery.id}`}>Details</Link></p>
          </div>
        </InfoWindow>) : null}
      </GoogleMap>
      </div>
