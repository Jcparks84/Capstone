...
const SearchPage = (props) => {
    ...
    async function getBrewery(search) {
        ...
        await getLatLng(response.data);
        console.log("ARRARRARARARARARA", arr)  
        const locationPromises = arr.map(location => axios.get(`https://maps.googleapis.com/maps/api/geocode/json?address=${location.address}&key=${apiKey}`));
        const locationResponses = await Promise.all(locationPromises);
     console.log('locationRespponse', locationResponses)
        const locns = locationResponses.map(locationResponse => locationResponse.data.results.length > 0 ? {lat: locationResponse.data.results[0].geometry.location.latitude, lng:locationResponse.data.results[0].geometry.location.longitude} : null);
        const updatedArray = arr.map((location, index) => ({ ...location, longitude: locns[index].lng | 0, latitude:  locns[index].lat | 0}));
