...
const SearchPage = (props) => {
    ...
    const breweryState = "%20" + brewery.state.replace(/\s/g, "%20")
	const [lat, setLat] = useState(37.0902)
    const [lng, setLng] = useState(-95.7129)
    const address = breweryAddress+breweryCity+breweryState
​
// Get the Lat & Lng of the Address
    const getLatLng = async () => {
        try {
            let response = await axios.get(`https://maps.googleapis.com/maps/api/geocode/json?address=${address}&key=`)
