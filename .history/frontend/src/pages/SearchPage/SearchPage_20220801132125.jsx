...
const SearchPage = (props) => {
    ...
    const getLatLng = (array) => {
        ...
        const locns = locationResponses.map(locationResponse => locationResponse.data.results.length > 0 ? {lat: imageResponse.data.results[0].geometry.location.latitude, lng:imageResponse.data.results[0].geometry.location.longitude} : null);
        const updatedArray = array.map((location, index) => ({ ...location, longitude: l[ccisnndex].lng, latitude:  locns[index].lat}));
         return updatedArray;
        
   }
   
   const modifiedBrewery = updatedArray(brewery);
